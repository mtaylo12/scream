  ...
  )
{
  ...
  if (comm.am_i_root()) {
    ...
    // NOTE, we are currently assuming the remap file was generated by NCO and
    // thus follows NCO conventions.
    // As such the dimensions have rather general names.  Here is an explanation:
    //   n_s is the total number of nonzero remap weights.
    //   n_a is the total number of columns in the SOURCE grid
    //     (i.e. in an ne2 -> ne4 remap this would have 218 columns)
    //   n_b is the total number of columns in the TARGET grid
    //     (i.e. in an ne2 -> ne4 remap this would have 866 columns)
    // Conceptually, if we wrote the remapping as y = W*x then W would be a matrix
    // with n_a columns, n_b rows and n_s total nonzero entries.
    // The data is stored in sparse format, so we have the following variables:
    //   S   is a vector of length n_s that represents all of the nonzero
    //       remapping weights
    //   col is a vector of length n_s that contains the indices of the SOURCE
    //       grid column associated with the appropriate weight in S
    //   row is a vector of length n_s that contains the indices of the TARGET
    //       grid column associated with the appropriate weight in S
    // Thus considering the matrix W described above.  If W[i,j] = w, and this was
    // the n'th nonzero weight in W then
    //   S[n]   = w
    //   col[N] = j
    //   row[N] = i
    ...
    EKAT_REQUIRE_MSG(target_ncols==ncols_scream,
      "ERROR: SPA get_remap_weights_from_file, remap target domain does not "
      "match simulation domain size");
    ...
    // Note, in remap files col -> row (src -> tgt)
    auto col_global_h = Kokkos::create_mirror_view(col_global);
    auto row_global_h = Kokkos::create_mirror_view(row_global);

    // Set up scorpio structures needed for input

    // Register variables for input
    std::vector<std::string> vec_of_dims = {"n_s"};
    std::string r_decomp = "Real-n_s";
    std::string i_decomp = "Int-n_s";
    scorpio::get_variable(remap_file_name, "S", "S", vec_of_dims.size(),
                          vec_of_dims, PIO_REAL, r_decomp);
    scorpio::get_variable(remap_file_name, "row", "row", vec_of_dims.size(),
                          vec_of_dims, PIO_INT, i_decomp);
    scorpio::get_variable(remap_file_name, "col", "col", vec_of_dims.size(),
                          vec_of_dims, PIO_INT, i_decomp);

    // Set the dof's to read in variables. Since all mpi ranks read in the full
    // set of data, the dof's are the whole array
  ...
  */
  ...
  // Note: This is an important step when running with multiple MPI ranks.
