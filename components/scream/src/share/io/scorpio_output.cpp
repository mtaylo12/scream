 ...
 */
  ...
  for (int i=0; i<layout.rank(); ++i) {
    ...
    const auto tag_name = get_io_tag_name(tags[i],dims[i]);
    ...
    std::cout << "Inserting tag " << tag_name << " from grid " << m_io_grid->name() << "\n";
...
void AtmosphereOutput::register_variables(const std::string& filename)
{
  ...
  for (auto const& name : m_fields_names) {
    ...
    for (int i=0; i<fid.get_layout().rank(); ++i) {
      const auto tag_name = get_io_tag_name(layout.tag(i), layout.dim(i));
...
std::string AtmosphereOutput::
get_io_tag_name (const FieldTag tag, const int extent) {
  ...
  return is_partitioned && m_io_grid->has_partitioned_dim_name()
        ? m_io_grid->get_partitioned_dim_name()
        : scorpio::get_nc_tag_name(tag,extent);
}
